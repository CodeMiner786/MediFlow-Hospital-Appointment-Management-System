...
{
    public class GetSettingsByIdHandler(IDoctorUnitOfWork uow, IMapper mapper)
    : IRequestHandler<GetSettingsByIdQuery, ApiResponseDto<HospitalSettingsDto>>
    {
        public async Task<ApiResponseDto<HospitalSettingsDto>> Handle(
            GetSettingsByIdQuery request, CancellationToken ct)
        {
