...
{
    public class GetDoctorsBySpecializationHandler(IDoctorUnitOfWork uow, IMapper mapper)
    : IRequestHandler<GetDoctorsBySpecializationQuery, ApiResponseDto<PagedResultDto<DoctorDto>>>
    {
        public async Task<ApiResponseDto<PagedResultDto<DoctorDto>>> Handle(
            GetDoctorsBySpecializationQuery request, CancellationToken ct)
        {
            var stream = uow.Doctors.GetDoctorsBySpecializationStream(request.Specialization);
