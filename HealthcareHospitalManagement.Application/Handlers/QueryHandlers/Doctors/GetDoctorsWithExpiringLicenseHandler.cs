...
{
    public class GetDoctorsWithExpiringLicenseHandler(IDoctorUnitOfWork uow, IMapper mapper)
    : IRequestHandler<GetDoctorsWithExpiringLicenseQuery, ApiResponseDto<PagedResultDto<DoctorDto>>>
    {
        public async Task<ApiResponseDto<PagedResultDto<DoctorDto>>> Handle(
            GetDoctorsWithExpiringLicenseQuery request, CancellationToken ct)
        {
            var stream = uow.Doctors.GetDoctorsWithExpiringLicenseStream(request.ThresholdDate);
