...
{
    public sealed class GetCurrentlyAdmittedPatientsQueryHandler(IWardEmergencyUnitOfWork uow, IMapper mapper)
    : IRequestHandler<GetCurrentlyAdmittedPatientsQuery, ApiResponseDto<PagedResultDto<AdmissionResponseDto>>>
    {
        public async Task<ApiResponseDto<PagedResultDto<AdmissionResponseDto>>> Handle(
            GetCurrentlyAdmittedPatientsQuery request, CancellationToken ct)
        {
            var allData = (await uow.Admissions.GetCurrentlyAdmittedPatientsAsync(ct)).ToList();
