...
{
    public sealed class GetAllWardsQueryHandler(IWardEmergencyUnitOfWork uow, IMapper mapper)
     : IRequestHandler<GetAllWardsQuery, ApiResponseDto<PagedResultDto<WardResponseDto>>>
    {
        public async Task<ApiResponseDto<PagedResultDto<WardResponseDto>>> Handle(
            GetAllWardsQuery request, CancellationToken ct)
        {
            var allData = (await uow.Wards.GetAllAsync(ct)).ToList();
