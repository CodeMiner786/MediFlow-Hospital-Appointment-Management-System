...
{
    public sealed class GetSessionsByDoctorQueryHandler(ITelemedicineUnitOfWork uow, IMapper mapper)
    : IRequestHandler<GetSessionsByDoctorQuery, ApiResponseDto<PagedResultDto<TelemedicineSessionResponseDto>>>
    {
        public async Task<ApiResponseDto<PagedResultDto<TelemedicineSessionResponseDto>>> Handle(
            GetSessionsByDoctorQuery request, CancellationToken ct)
        {
