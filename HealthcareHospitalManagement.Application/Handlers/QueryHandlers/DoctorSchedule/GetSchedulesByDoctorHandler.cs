...
{
    public class GetSchedulesByDoctorHandler(IDoctorUnitOfWork uow, IMapper mapper)
    : IRequestHandler<GetSchedulesByDoctorQuery, ApiResponseDto<PagedResultDto<DoctorScheduleDto>>>
    {
        public async Task<ApiResponseDto<PagedResultDto<DoctorScheduleDto>>> Handle(
            GetSchedulesByDoctorQuery request, CancellationToken ct)
        {
            var stream = uow.DoctorSchedules.GetSchedulesByDoctorStream(request.DoctorId);
