...
{
    public class GetStaffWithDetailsHandler(IDoctorUnitOfWork uow, IMapper mapper)
    : IRequestHandler<GetStaffWithDetailsQuery, ApiResponseDto<StaffDto>>
    {
        public async Task<ApiResponseDto<StaffDto>> Handle(
            GetStaffWithDetailsQuery request, CancellationToken ct)
        {
            var entity = await uow.Staff.GetStaffWithDetailsAsync(request.StaffId);
            if (entity is null)
