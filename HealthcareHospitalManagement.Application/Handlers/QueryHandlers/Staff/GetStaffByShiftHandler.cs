...
{
    public class GetStaffByShiftHandler(IDoctorUnitOfWork uow, IMapper mapper)
    : IRequestHandler<GetStaffByShiftQuery, ApiResponseDto<PagedResultDto<StaffDto>>>
    {
        public async Task<ApiResponseDto<PagedResultDto<StaffDto>>> Handle(
            GetStaffByShiftQuery request, CancellationToken ct)
        {
            var stream = uow.Staff.GetStaffByShiftStream(request.ShiftType);
